...
namespace FreelanceStudent.Core.Dtos
{
    public class CustomResponse<T>
    {
        ...
        {
            return new CustomResponse<T> { Data = default(T), Errors = errors, StatusCode = statusCode, IsSuccessful = false };
        }

        public static CustomResponse<T> Fail(string error, int statusCode)
        {
            return new CustomResponse<T> { Data = default(T), Errors = new List<string> { error }, StatusCode = statusCode, IsSuccessful = false };
        }
    }
}
